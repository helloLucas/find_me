...
public class MinigameSessionService {
  ...
  public String createSession(Long userId, String fragmentCode) {
    ...
    log.info(
        "Minigame session created for user: {}, fragment: {}, session: {}",
        userId,
        fragmentCode,
        sessionId);
    return sessionId;
  ...
  public boolean verifySession(
      Long userId, String fragmentCode, String sessionId, long minDurationMs) {
    String key = resolveKey(userId, fragmentCode);
    String value = redisTemplate.opsForValue().get(key);

    if (value == null) {
      log.warn(
          "Session verification failed: No session found for user {} and fragment {}",
          userId,
          fragmentCode);
      return false;
    }
    ...
    if (parts.length != 2) {
      log.error(
          "Session verification failed: Invalid session format in Redis for user {} and fragment {}",
          userId,
          fragmentCode);
      return false;
    }
    ...
    if (!storedSessionId.equals(sessionId)) {
      log.warn(
          "Session verification failed: Session ID mismatch. Expected {}, but got {}",
          storedSessionId,
          sessionId);
      return false;
    }
    ...
    if (duration < minDurationMs) {
      log.warn(
          "Session verification failed: Playtime too short. Duration: {}ms, Required: {}ms",
          duration,
          minDurationMs);
      return false;
    }
    ...
    log.info(
        "Session verified successfully for user {} and fragment {}. Duration: {}ms",
        userId,
        fragmentCode,
        duration);
