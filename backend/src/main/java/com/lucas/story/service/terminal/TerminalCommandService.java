...
public class TerminalCommandService {
    ...
    private static final List<String> DANGEROUS_COMMANDS = Arrays.asList(
        "rm", "mv", "cp", "chmod", "chown", "sudo", "su", "apt", "yum", "wget", "curl"
    );
    ...
    private TerminalResult handleLs(ParsedCommand command, String cwd, VfsContext vfs) {
        String targetPath = command.args().isEmpty() ? cwd : pathResolver.resolve(cwd, command.args().get(0), vfs.getRootPath());
        VfsNode node = vfs.resolve(targetPath);

        if (node == null) {
            return buildErrorResult(cwd, vfs, "ls: " + command.args().get(0) + ": No such file or directory");
        }
        ...
        List<String> output = children.stream()
            .filter(n -> !n.hidden())
            .map(VfsNode::name)
            .collect(Collectors.toList());

        return TerminalResult.builder()
            .stdout(output.isEmpty() ? new ArrayList<>() : Collections.singletonList(String.join("  ", output)))
            .cwd(cwd)
            .prompt(buildPrompt(cwd, vfs))
            .resultCode("SUCCESS")
            .build();
    ...
    private TerminalResult handleCd(ParsedCommand command, String cwd, VfsContext vfs) {
        ...
        if (node == null) {
            return buildErrorResult(cwd, vfs, "cd: " + command.args().get(0) + ": No such file or directory");
        }
    ...
    private TerminalResult handleCat(ParsedCommand command, String cwd, VfsContext vfs) {
        ...
        if (node == null) {
            return buildErrorResult(cwd, vfs, "cat: " + command.args().get(0) + ": No such file or directory");
        }
    ...
    private TerminalResult handleSh(ParsedCommand command, String cwd, VfsContext vfs) {
        ...
        if (node == null) {
            return buildErrorResult(cwd, vfs, "sh: " + command.args().get(0) + ": No such file or directory");
        }

        // lukas_fragment_01.sh 전용 에러 메시지 (사용자 요구사항)
        if ("LUCAS_FRAGMENT_01".equals(node.contentKey())) {
            return TerminalResult.builder()
                .stderr(Arrays.asList(
