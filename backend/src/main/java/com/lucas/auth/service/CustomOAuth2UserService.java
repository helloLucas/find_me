...
/**
 * OAuth2 인증 완료 후, 사용자 정보를 DB와 대조하여
 * [로그인 / 게스트 -> 멤버 전환 / 신규 가입] 중 하나를 처리하는 서비스 클래스입니다.
 */
@Slf4j
@Service
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        /**
         * userRequest에서 registrationId 추출 후 registrationId로 AuthProvider 저장
     ...
     * 만약 찾은 회원이 있다면 그대로 반환하고, 없다면 게스트 계정에서의 전환 또는 신규 저장을 수행한다.
     *
    ...
    private User getUser(OAuthAttributes attributes, AuthProvider provider) {
        ...
        return userRepository.findByProviderAndProviderUserId(provider, providerUserId)
            .map(user -> {
                log.info("기존 회원 로그인 - provider={}, providerUserId={}, userId={}",
     ...
     * @return 승격된 User 객체
     */
    private User upgradeGuestToMember(Long guestId, OAuthAttributes attributes, AuthProvider provider) {
        log.info("게스트 -> 멤버 전환 시도 - guestId={}, email={}", guestId, attributes.getEmail());

        // 게스트의 정보를 회원 정보로 덮어쓰고 role을 MEMBER로 전환
        User guest = userRepository.findById(guestId)
            .orElseThrow(() -> new com.lucas.global.exception.CustomException(com.lucas.global.exception.ErrorCode.E3000));
