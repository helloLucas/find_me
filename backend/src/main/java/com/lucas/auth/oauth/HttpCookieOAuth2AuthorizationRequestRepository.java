 ...
 *
 * Spring Security의 기본 구현체({@code HttpSessionOAuth2AuthorizationRequestRepository})는 서버 세션을
 * 이용하여 OAuth2 상태(state)를 관리합니다. Stateless(JWT) 환경 또는 로드 밸런서가 있는 분산 환경에서는 세션 불일치로
 * 인해 간헐적인 {@code access_denied} 에러가 발생할 수 있습니다.
 *
 * 이 클래스는 OAuth2 인증 요청 정보를 Base64+JSON 직렬화하여 쿠키에 저장하고, 콜백 시 쿠키에서 역직렬화하여 세션 없이 상태를 복원합니다.
 */
@Slf4j
@Component
public class HttpCookieOAuth2AuthorizationRequestRepository
  ...
  /**
   * OAuth2 인증 요청 상태를 쿠키에 저장합니다.
   * authorizationRequest가 null이면 쿠키를 삭제합니다.
   *
  ...
  /**
   * OAuth2AuthorizationRequest 를 Base64(JSON) 문자열로 직렬화합니다.
   * OAuth2AuthorizationRequest 자체가 직접 직렬화 불가능하므로 필요한 필드만 Map으로 추출합니다.
   */
  ...
  private OAuth2AuthorizationRequest deserialize(String value) {
    try {
      ...
      return OAuth2AuthorizationRequest.authorizationCode()
          ...
          .scopes(((List<String>) data.get("scopes") != null)
              ? new HashSet<>((List<String>) data.get("scopes"))
              : Collections.emptySet())
          .state((String) data.get("state"))
          .additionalParameters((Map<String, Object>) data.getOrDefault("additionalParameters", new HashMap<>()))
  ...
  private void deleteCookie(HttpServletResponse response) {
    ResponseCookie expiredCookie = ResponseCookie.from(OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME, "")
        .httpOnly(true)
        .secure(true)
        .path("/")
        .maxAge(0)
