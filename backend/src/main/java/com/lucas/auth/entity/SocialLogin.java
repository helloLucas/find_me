 ...
 *
 * 하나의 {@link User}가 여러 소셜 제공자(Google, SSAFY 등)로 로그인할 수 있도록
 * 유저 본체와 인증 수단을 1:N으로 분리합니다.
 */
@Entity
@Getter
@NoArgsConstructor
@Table(name = "social_logins", uniqueConstraints = {
    @UniqueConstraint(name = "uk_social_logins_provider_provider_user_id", columnNames = { "provider",
        "provider_user_id" })
})
